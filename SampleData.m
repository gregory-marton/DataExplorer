function T = SampleData(filepath, nrows, options)
%SAMPLEDATA  Uniform random sample from a large CSV/TSV/text file.
%
...
%   Usage
%   ─────
%   T = SampleData('ebirddata.tsv')          % 10 000 rows (default)
%   T = SampleData('ebirddata.tsv', 50000)   % 50 000 rows
%   T = SampleData('bigfile.csv', 10000, Seed=42)   % reproducible sample
%
%   The result is a MATLAB table you can pass directly to DataExplorer:
%       T = SampleData('ebirddata.tsv', 10000);
%       DataExplorer(T);
%   or save for later:
...
%% ── Validate input ────────────────────────────────────────────────────────────
if ~isfile(filepath)
    error('SampleData:notFound', 'File not found: %s', filepath);
end
...
try
    ...
    error('SampleData:datastoreError', ...
...
while hasdata(ds)
    ...
    if isempty(reservoir) && n_chunk < k
        % Haven't filled the reservoir yet — accumulate
        reservoir = chunk;
        n_seen    = n_chunk;
        continue
    end
    ...
    if isempty(reservoir)
        % First chunk big enough to fill — seed the reservoir
        reservoir = chunk(1:k, :);
        n_seen    = k;
        start_row = k + 1;
    else
    ...
    % Process remaining rows in this chunk
...
%% ── Verify column names ───────────────────────────────────────────────────────
% If all names are Var1/Var2/…, the header row may have been missed.
% Report this clearly rather than silently returning unusable data.
