...
# ── Cities (cities) — ~1M+ population, geographic tile layout ────────────────
# Rows 0-7 = north to south; cols 0-22 = west to east.
# Cols 0-5 Americas │ 6 gap │ 7-13 Europe/Africa/MiddleEast │ 14-17 CentralAsia
# Cols 18-20 EastAsia │ 21-22 SEAsia/Oceania
write_grid("cities", [
    # ── North America ──────────────────────────────────────────────────────────
    ("VAN",  0,  0, ["Vancouver", "Metro Vancouver"]),
    ("MON",  0,  5, ["Montréal", "Montreal"]),
    ("SEA",  1,  0, ["Seattle", "Seattle-Tacoma"]),
    ("CHI",  1,  2, ["Chicago", "Chicagoland"]),
    ("TOR",  1,  3, ["Toronto", "Greater Toronto"]),
    ("NYC",  1,  4, ["New York", "New York City", "NYC"]),
    ("BOS",  1,  5, ["Boston"]),
    ("SFO",  2,  0, ["San Francisco", "SF", "Bay Area"]),
    ("PHX",  2,  1, ["Phoenix"]),
    ("ATL",  2,  3, ["Atlanta"]),
    ("MIA",  2,  4, ["Miami"]),
    ("LAX",  3,  0, ["Los Angeles", "LA"]),
    ("MEX",  3,  1, ["Mexico City", "Ciudad de México", "CDMX"]),
    ("HOU",  3,  2, ["Houston"]),
    ("HAV",  3,  3, ["Havana", "La Habana"]),
    ("SDO",  3,  4, ["Santo Domingo"]),
    ("GDL",  4,  1, ["Guadalajara"]),
    ("BOG",  4,  2, ["Bogotá", "Bogota"]),
    ("CAR",  4,  3, ["Caracas"]),
    ("LIM",  5,  1, ["Lima"]),
    ("FOR",  5,  3, ["Fortaleza"]),
    ("REC",  5,  4, ["Recife"]),
    ("SCL",  6,  0, ["Santiago", "Santiago de Chile"]),
    ("SAO",  6,  2, ["São Paulo", "Sao Paulo"]),
    ("RIO",  6,  3, ["Rio de Janeiro"]),
    ("BUE",  7,  1, ["Buenos Aires", "Ciudad de Buenos Aires", "CABA"]),
    # ── Europe ─────────────────────────────────────────────────────────────────
    ("OSL",  0,  8, ["Oslo"]),
    ("STO",  0,  9, ["Stockholm"]),
    ("HEL",  0, 10, ["Helsinki"]),
    ("SPB",  0, 11, ["Saint Petersburg", "St. Petersburg", "Leningrad"]),
    ("DUB",  1,  7, ["Dublin"]),
    ("LON",  1,  8, ["London", "Greater London"]),
    ("PAR",  1,  9, ["Paris", "Île-de-France"]),
    ("BER",  1, 10, ["Berlin"]),
    ("WAR",  1, 11, ["Warsaw", "Warszawa"]),
    ("MOS",  1, 12, ["Moscow", "Москва"]),
    ("MAD",  2,  7, ["Madrid"]),
    ("BCN",  2,  8, ["Barcelona"]),
    ("ROM",  2,  9, ["Rome", "Roma"]),
    ("MIL",  2, 10, ["Milan", "Milano"]),
    ("VIE",  2, 11, ["Vienna", "Wien"]),
    ("IST",  2, 12, ["Istanbul", "Constantinople"]),
    ("KYV",  2, 13, ["Kyiv", "Kiev"]),
    # ── North Africa & Middle East ─────────────────────────────────────────────
    ("CAS",  3,  7, ["Casablanca", "Dar el Beida"]),
    ("ALG",  3,  8, ["Algiers", "Alger"]),
    ("CAI",  3, 12, ["Cairo", "Al-Qahirah"]),
    ("BAG",  3, 13, ["Baghdad", "Baghdād"]),
    ("TEH",  3, 14, ["Tehran", "Teheran"]),
    ("RIY",  4, 13, ["Riyadh", "Riyādh"]),
    # ── Sub-Saharan Africa ─────────────────────────────────────────────────────
    ("DAK",  4,  7, ["Dakar"]),
    ("LAG",  4,  8, ["Lagos"]),
    ("KAN",  4,  9, ["Kano"]),
    ("KHR",  4, 11, ["Khartoum", "Kharṭūm"]),
    ("ADD",  4, 12, ["Addis Ababa", "Addis Abeba", "Finfinne"]),
    ("ABJ",  5,  7, ["Abidjan"]),
    ("ACC",  5,  8, ["Accra"]),
    ("KIN",  5, 10, ["Kinshasa", "Léopoldville"]),
    ("NAI",  5, 11, ["Nairobi"]),
    ("DAR",  5, 12, ["Dar es Salaam"]),
    ("LUA",  6,  9, ["Luanda"]),
    ("LUS",  6, 11, ["Lusaka"]),
    ("HRR",  6, 12, ["Harare", "Salisbury"]),
    ("CPT",  7, 10, ["Cape Town", "Kaapstad", "iKapa"]),
    ("JNB",  7, 11, ["Johannesburg", "Joburg", "Egoli"]),
    ("DRB",  7, 12, ["Durban", "eThekwini"]),
    # ── Central & South Asia ──────────────────────────────────────────────────
    ("TAS",  2, 15, ["Tashkent", "Toshkent"]),
    ("KHI",  3, 15, ["Karachi"]),
    ("DEL",  3, 16, ["Delhi", "New Delhi"]),
    ("DHK",  3, 17, ["Dhaka", "Dacca"]),
    ("MUM",  4, 15, ["Mumbai", "Bombay"]),
    ("HYD",  4, 16, ["Hyderabad"]),
    ("CHE",  4, 17, ["Chennai", "Madras"]),
    ("BLR",  5, 15, ["Bangalore", "Bengaluru"]),
    ("COL",  5, 16, ["Colombo"]),
    # ── East Asia ─────────────────────────────────────────────────────────────
    ("BEI",  2, 18, ["Beijing", "Peking", "北京"]),
    ("SEO",  2, 19, ["Seoul", "서울"]),
    ("TKY",  2, 20, ["Tokyo", "東京", "Tōkyō"]),
    ("SHA",  3, 18, ["Shanghai", "上海"]),
    ("OSA",  3, 19, ["Osaka", "大阪"]),
    ("GZH",  4, 18, ["Guangzhou", "Canton", "广州"]),
    ("HKG",  4, 19, ["Hong Kong", "香港"]),
    ("TAI",  4, 20, ["Taipei", "臺北"]),
    # ── Southeast Asia ────────────────────────────────────────────────────────
    ("HAN",  3, 21, ["Hanoi", "Hà Nội"]),
    ("BKK",  4, 21, ["Bangkok", "Krung Thep", "กรุงเทพ"]),
    ("MNL",  4, 22, ["Manila", "Metro Manila"]),
    ("RNG",  5, 17, ["Yangon", "Rangoon"]),
    ("HCM",  5, 19, ["Ho Chi Minh City", "Saigon", "HCMC"]),
    ("KUL",  5, 20, ["Kuala Lumpur", "KL"]),
    ("SIN",  5, 21, ["Singapore"]),
    ("JAK",  6, 20, ["Jakarta", "Batavia"]),
    # ── Oceania ───────────────────────────────────────────────────────────────
    ("BRI",  6, 22, ["Brisbane"]),
    ("PER",  7, 20, ["Perth"]),
    ("MEL",  7, 21, ["Melbourne"]),
